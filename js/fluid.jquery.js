...
(function($){
	$.fn.fluid = function(options) {
		...
		function positonEls() {
			...
			obj.container.find('.item').each(function(i) {
				...
				_getXY($item, i, function(left, top) {
					...
					$item.attr('old-left',left).attr('old-top',top);
					if($item.hasClass('rez-test-show')){
						
						$item.attr('new-left',left).attr('new-top',top);
					}
		...
		obj.col = options.col,
		obj.spacingX = options.spacingX || 0; // defaults
		obj.spacingY = options.spacingY || 0; // defaults	
		obj.container = this,
